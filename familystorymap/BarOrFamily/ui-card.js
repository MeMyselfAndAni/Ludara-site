...
function _populateCard(p){
  ...
  /* A place may name its own link: websiteLabel is a trilingual string, so a
     card can offer "The book" or "The museum" rather than a bare "Website".
     Without it nothing changes. (12 September 2026.) */
  if(p.website){
    var _wl = p.websiteLabel
      ? ((typeof pickLang === 'function') ? pickLang(p.websiteLabel) : p.websiteLabel)
      : 'Website';
    contacts += `<a class="pc-contact-pill" href="${p.website}" target="_blank" rel="noopener">🌐 ${_wl}</a>`;
  }
