...
function openTripInMaps(){
  try {
    ...
    // Use coordinates (not place names) so Google Maps lands on the exact spot.
    // Garden features like "March Bank" are not searchable by name in Google Maps.
    // Holston pattern: use each stop's curated `search` string when present (Google
    // resolves the named venue); otherwise fall back to lat,lng so garden features
    // like "March Bank" (not searchable by name) still land on the exact spot.
