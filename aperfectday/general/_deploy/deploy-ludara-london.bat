@echo off
REM ============================================================
REM  deploy-ludara-london.bat
REM  Deploys A Perfect Day — London (neutral city guide, no blogger)
REM
...
if not exist "%DEST%" (
  ...
  echo        Create aperfectday\ludara\london\ and add map.js, index.html, data.js, images\ first.
  pause & exit /b 1
)
...
cd /d "%REPO_ROOT%"
git add -A
git commit -m "Deploy ludara/london — A Perfect Day neutral city guide"
git push

echo.
echo ==========================================================
echo  Done! Live in ~30s at:
echo  https://ludara.ai/aperfectday/ludara/london/
echo ==========================================================
echo.
pause
