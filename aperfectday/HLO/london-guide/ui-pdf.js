  ...
  const cards = places.map((p, i) => {
    ...
    <div class="pdf-card">
      ...
      <div class="pdf-card-body">
        ...
        ${p.note ? `<div class="pdf-card-note">"${p.note}"<span class="pdf-note-by"> — ${GUIDE.blogger.authors}, ${GUIDE.blogger.name}</span></div>` : ''}
        ${p.tip ? `<div class="pdf-card-tip"><span class="pdf-tip-label">💡 ${GUIDE.blogger.authors}' Tip</span> ${p.tip}</div>` : ''}
...
<head>
<meta charset="UTF-8">
<title>A Perfect Day · ${GUIDE.city.name}</title>
...
<!-- COVER -->
<div class="pdf-cover">
  <div class="pdf-cover-logo">${GUIDE.blogger.name} · A Perfect Day by Ludara</div>
  <div class="pdf-cover-title">${GUIDE.city.name}</div>
  ...
  <div class="pdf-cover-by">Curated by ${GUIDE.blogger.authors}</div>
