...
function _updateFavBtn(){
  ...
  const favs = JSON.parse(localStorage.getItem('london-favs') || '[]');
...
function cardToggleFav(){
  ...
  let favs = JSON.parse(localStorage.getItem('london-favs') || '[]');
  if(favs.includes(CARD_PLACE.id)){
    favs = favs.filter(id => id !== CARD_PLACE.id);
  } else {
    favs.push(CARD_PLACE.id);
  }
  localStorage.setItem('london-favs', JSON.stringify(favs));
  _updateFavBtn();
