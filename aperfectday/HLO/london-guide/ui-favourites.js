...
let favourites = JSON.parse(localStorage.getItem('london-favs') || '[]');
let savedFilterActive = false;

function saveFavs(){
  localStorage.setItem('london-favs', JSON.stringify(favourites));
  updateFavUI();
}

// Sync in-memory favourites from localStorage (called after external writes)
function refreshFavourites(){
  favourites = JSON.parse(localStorage.getItem('london-favs') || '[]');
  updateFavUI();
}
